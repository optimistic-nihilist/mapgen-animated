...
fn render_help_full(params: TextParams) {
    const HELP_TEXT: [&str; 4] = [
        ...
        "[f] - toggle FPS     [3] - random walk     [ESC] - quit",
        "[h] - toggle help    [4] - cell. automata",
    ];
...
async fn main() {
    ...
    loop {
        ...
        if is_key_pressed(KeyCode::Key1) {
            let algo = TunnelingGenerator {};
            map = new_map(TileType::Wall);
            rooms.clear();
            render_map(&tiles, texture, &map);
            for _ in 0..TUNNELING_MAX_ROOMS {
                let mut placed_room = false;
                while placed_room != true {
        ...
        if is_key_pressed(KeyCode::Key6) {
            map = new_map(TileType::Wall);
            MazeGenerator::generate_map(&mut map, &tiles, texture).await
        }
